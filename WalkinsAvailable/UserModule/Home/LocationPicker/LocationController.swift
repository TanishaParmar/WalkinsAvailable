...
//        }
        ...
        if let location = self.defaultSearchCoordinate {
            self.mapView(isHidden: false)
            self.searchBar.isHidden = true
            self.resultView.setPreLocationView(isView: true)
            let clLocation = CLLocation(latitude: location.latitude, longitude: location.longitude)
            
            let viewRegion = MKCoordinateRegion(center: location, latitudinalMeters: 500, longitudinalMeters: 500)
            self.mapView.setRegion(viewRegion, animated: false)
            
            self.reverseGeocodeLocation(clLocation)
        } else {
            self.searchBar.isHidden = false
            self.resultView.setPreLocationView(isView: false)
...
//        view.addSubview(searchBar)
    ...
    private func layoutViews() {
        ...
        let searchBar = UIStackView(arrangedSubviews: [self.searchBar])
        searchBar.translatesAutoresizingMaskIntoConstraints = false
        searchBar.axis = .vertical
