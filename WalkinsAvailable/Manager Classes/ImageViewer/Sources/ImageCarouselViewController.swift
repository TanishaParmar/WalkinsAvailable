...
class ImageCarouselViewController: UIPageViewController, ImageViewerTransitionViewControllerConvertible {
    ...
    var theme:ImageViewerTheme = .blur {
        didSet {
    ...
    private(set) lazy var backgroundView: UIVisualEffectView? = {
        ...
        let _v = UIVisualEffectView(effect: blurEffect)
    ...
    private func addNavBar() {
        ...
        btn.addTarget(nil, action: #selector(dismiss(_:)), for: .touchUpInside)
        btn.setTitle("Close", for: .normal)
        btn.setTitleColor(theme.closeButtonForgourndColor, for: .normal)
        let closeBarButton = UIBarButtonItem(customView: btn)
        ...
        navBar.backgroundColor = .clear // UIColor.black.withAlphaComponent(0.15)
    }
